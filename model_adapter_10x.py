...
def get_model_configs(model_name=None):
    ...
    base_config = {
        ...
        'd_model': 2048,  # 缩小为5倍: 256 -> 1280 (原10x的一半)
        'n_heads': 32,    # 缩小为5倍: 8 -> 40 (原10x的一半)
        'd_ff': 2048,     # 缩小为5倍: 512 -> 2560 (原10x的一半)
        'e_layers': 4,   # 缩小为5倍: 2 -> 10 (原10x的一半)
        'd_layers': 4,    # 缩小为5倍: 1 -> 5 (原10x的一半)
        'dropout': 0.1,
        'activation': 'gelu',
        ...
        'use_norm': True,
        'd_state': 80,   # 缩小为5倍: 16 -> 80 (原10x的一半)
    ...
    model_specific_configs = {
        ...
        # s_mamba的特殊配置：d_model应该是特征维度39，但内部使用195
        's_mamba': {
            'd_model': 195,  # 对于s_mamba，d_model缩小为5倍: 39 -> 195 (原10x的一半)
            'd_ff': 1280,    # 缩小为5倍: 256 -> 1280 (原10x的一半)
            'e_layers': 10,  # 缩小为5倍: 2 -> 10 (原10x的一半)
            'activation': 'relu',
            'use_norm': True,
            'embed': 'timeF',
            'freq': 'd'
...
if __name__ == "__main__":
    ...
    # 测试10倍参数配置
    print("\n10倍参数配置示例:")
    for model_name in ['Transformer', 'Autoformer', 'Informer', 's_mamba']:
        config = get_model_configs(model_name)
        print(f"  {model_name}: d_model={config['d_model']}, d_ff={config['d_ff']}, e_layers={config['e_layers']}")
