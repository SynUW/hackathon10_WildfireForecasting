...
class YearTimeSeriesPixelDataset(Dataset):
    ...
    def __init__(self, h5_dir, years=None, return_metadata=True, 
        ...
        if enable_performance_optimizations:
            ...
            logger.info(f"🚀 Performance optimizations enabled: File handle cache({max_file_handles}), Data cache({data_cache_size_mb}MB)")
        else:
        ...
        if enable_performance_optimizations:
            ...
            logger.info(f"📊 Performance cache status: Data cache {cache_stats['entries']} entries, {cache_stats['size_mb']:.1f}MB")
    ...
    def _get_cache_filename(self):
        ...
        # Generate unique identifier based on key parameters
        cache_params = {
            'h5_dir': self.h5_dir,
            ...
            with h5py.File(file_path, 'r') as f:
                ...
                for pixel_name in pixel_names:
                    try:
                        ...
                        # 🔍 If fire days found, add debug information
                        if len(fire_days) > 0:
    ...
    def _sample_data_with_cache(self):
        ...
        logger.info("💾 Cache invalid, starting resampling...")
        
        # Record start time
        start_time = time.time()
        
        # Find global no-fire days
        logger.info("🔍 Step 1: Finding global no-fire days...")
        step_start = time.time()
        global_no_fire_days = self._find_global_no_fire_days()
        step_duration = time.time() - step_start
        logger.info(f"✅ Completed, time: {step_duration:.2f}s")
        
        # Find positive samples
        logger.info("🔍 Step 2: Finding positive samples...")
        step_start = time.time()
        ...
        step_start = time.time()
        num_negative_samples = len(positive_samples) * 2  # Default 2x negative samples
        negative_samples = self._generate_negative_samples(global_no_fire_days, num_negative_samples)
        step_duration = time.time() - step_start
        logger.info(f"✅ Completed, time: {step_duration:.2f}s")
        
        # Save to cache
        logger.info("💾 Step 4: Saving to cache...")
        step_start = time.time()
        ...
        logger.info(f"🎉 Sampling completed! Total time: {total_duration:.2f}s")
        logger.info(f"📊 Sampling results: {len(positive_samples)} positive samples, {len(negative_samples)} negative samples")
    ...
    def _apply_sample_ratio_filtering(self, seed=None):
        """Apply positive-negative sample ratio filtering"""
        if seed is None:
            seed = 42
        
        if self.verbose_sampling:
            logger.info(f"Starting positive-negative sample ratio filtering... (random seed: {seed})")
        ...
        if self.verbose_sampling:
            logger.info(f"Original sample statistics: {positive_count} positive samples, {negative_count} negative samples")
        
        # Calculate samples to retain
        ...
        if self.verbose_sampling:
            logger.info(f"Plan to retain: {retained_positive_count} positive samples, {retained_negative_count} negative samples")
        ...
        if self.verbose_sampling:
            logger.info(f"Sample filtering completed: {len(selected_positive_indices)} positive samples, {len(selected_negative_indices)} negative samples")
    ...
    def clear_performance_caches(self):
        ...
        if self.enable_performance_optimizations:
            ...
            logger.info("⚠️  Performance optimizations not enabled, no need to clear caches")
...
def test_year_dataloader():
    ...
    try:
        ...
        logger.info("📊 Test 1: Performance optimization enabled...")
        start_time = time.time()
        ...
        logger.info(f"⏰ Optimized data loader initialization time: {init_time_optimized:.2f}s")
        ...
        logger.info("📊 Test 2: Performance optimization disabled comparison...")
        start_time = time.time()
        ...
        logger.info(f"⏰ Non-optimized data loader initialization time: {init_time_no_opt:.2f}s")
        logger.info(f"🚀 Initialization speedup: {init_time_no_opt/init_time_optimized:.2f}x")
        
        # Test 3: Year-based split
        logger.info("📊 Test 3: Year-based split...")
        ...
        logger.info("📊 Test 4: Data loading performance comparison...")
        if train_indices:
            ...
            logger.info(f"⏰ Optimized single sample loading time: {sample_time_optimized:.4f}s")
            logger.info(f"⏰ Non-optimized single sample loading time: {sample_time_no_opt:.4f}s")
            logger.info(f"🚀 Single sample loading speedup: {sample_time_no_opt/sample_time_optimized:.2f}x")
            
            logger.info(f"✅ Sample shape: past={past_data.shape}, future={future_data.shape}")
            logger.info(f"✅ Metadata: {metadata}")
            
            # Check data quality
            logger.info("📊 Test 5: Data quality check...")
            logger.info(f"✅ Past data range: [{past_data.min():.4f}, {past_data.max():.4f}]")
            logger.info(f"✅ Future data range: [{future_data.min():.4f}, {future_data.max():.4f}]")
            logger.info(f"✅ NaN check: Past={torch.isnan(past_data).sum()}, Future={torch.isnan(future_data).sum()}")
            
            # Validate data consistency
            logger.info("📊 Test 6: Data consistency validation...")
            data_equal = torch.allclose(past_data, past_data_no_opt, rtol=1e-5, atol=1e-8)
            logger.info(f"✅ Data consistency before and after optimization: {'Passed' if data_equal else 'Failed'}")
        
        # Test 7: Optimized DataLoader batch performance
        logger.info("📊 Test 7: Optimized DataLoader batch performance...")
        ...
        for i, batch in enumerate(train_loader_optimized):
            ...
            if i == 0:
                logger.info(f"✅ Batch shape: past={past_batch.shape}, future={future_batch.shape}, metadata={len(metadata_batch)}")
            if i >= 4:  # Only test first 5 batches
                break
        
        batch_time_optimized = time.time() - start_time
        logger.info(f"⏰ Optimized batch processing time: {batch_time_optimized:.2f}s (5 batches)")
        ...
        logger.info(f"⏰ Non-optimized batch processing time: {batch_time_no_opt:.2f}s (5 batches)")
        logger.info(f"🚀 Batch processing speedup: {batch_time_no_opt/batch_time_optimized:.2f}x")
        
        # Test 8: Sample statistics
        logger.info("📊 Test 8: Sample statistics...")
        stats = data_loader.dataset.get_current_sample_stats()
        logger.info(f"✅ Sample statistics: {stats}")
        
        # Test 9: Cache effect
        logger.info("📊 Test 9: Cache effect...")
        start_time = time.time()
        ...
        logger.info(f"⏰ Cache loading time: {cache_time:.2f}s")
        logger.info(f"🚀 Cache speedup: {init_time_optimized/cache_time:.1f}x")
        
        # Test 10: Performance cache statistics
        logger.info("📊 Test 10: Performance cache statistics...")
        final_perf_stats = data_loader.dataset.get_performance_stats()
        logger.info(f"📊 Final performance statistics: {final_perf_stats}")
        
        # Test cache clearing
        logger.info("📊 Test 11: Cache clearing...")
        data_loader.dataset.clear_performance_caches()
        cleaned_stats = data_loader.dataset.get_performance_stats()
        logger.info(f"📊 Performance statistics after clearing: {cleaned_stats}")
