...
class Model(nn.Module):
    def __init__(self, conifgs):
        ...
        self.num_blocks = 2 # conifgs.freq_num_blocks
        self.exeprt_num = 3 # conifgs.expert_num
        self.dropout = 0.1 # conifgs.dropout_freq
    ...
    def forward(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None):
        # x_enc: (B, L, N) - 输入序列
        # 其他参数保持兼容性，但在此模型中不使用
        x = x_enc  # 使用编码器输入作为主要输入
        
        # 2. 将 -9999 替换为 0
        x = torch.where(x == -9999, torch.zeros_like(x), x)
        
        
        x, bound, weight = self.moe(x.permute(0, 2, 1))
        x = x.permute(0, 2, 1)
        residual = x
        total_prediction = torch.zeros(x.shape[0], self.pred_len, x.shape[2], device=x.device)
...
if __name__ == "__main__":
    ...
    class Configs:
        def __init__(self):
            ...
            self.enc_in = 8        # 通道数
            self.freq_num_blocks = 2
            self.expert_num = 4
            self.dropout_freq = 0.1
        
    # 配置
    configs = Configs()
    model = Model(configs)

    # 随机数据 (batch=32, seq_len=96, channels=8)
    x = torch.randn(32, configs.seq_len, configs.enc_in)
    y = torch.randn(32, configs.pred_len, configs.enc_in)
